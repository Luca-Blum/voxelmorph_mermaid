...
class Networks:
	...
	def __init__(self, datahandler, losses=[vxm.losses.NCC().loss, vxm.losses.Grad('l2').loss], loss_weights=[1, 0.01], train_size = 0.8):
		self.dh = datahandler
	...
	def _create_train_test_split(self,train_size):
		...
		self.vol_shape = self.vol_shape + np.array([self.padding[1][0]+self.padding[1][1],self.padding[2][0]+self.padding[2][1],self.padding[3][0]+self.padding[3][1]])
	...
	def vxm_data_generator(self, x_data, batch_size=32):
		#Adapted from voxelmorph tutorial : https://colab.research.google.com/drive/1WiqyF7dCdnNBIANEY80Pxw_mVz4fyV-S?usp=sharing#scrollTo=dJcCEElPPXN2
		"""
	...
	def train_vxm(self):
		...
		# train model
		hist = self.vxm_model.fit(train_generator, callbacks=[callback], epochs=epochs, steps_per_epoch=steps_per_epoch, verbose=2)
	...
	def load_vxm(self, model_path=''):
		...
		if model_path == '':
			...
			all_models = [Path(join(model_path, f)).name for f in listdir(model_path) if isdir(join(model_path, f)) and 'vxm_model' in f]
	...
	def train_mermaid(self):
		...
		print(prep_data_path)
		print(isfile(prep_data_path))
	
		print(train_path)
		print(isfile(train_path))
		print(eval_path)
		print(isfile(eval_path))
	...
	def evaluate_axes_vxm(self, test_input, test_output, postfix=''):
		...
		print("volume " + str(vol_shape))
			
		print("number of pairs: " + str(len(test_input)))
			
		titles = ['fixed sagital', 'fixed coronal', 'fixed traverse', 'warped sagital', 'warped coronal', 'warped traverse']
		
		for i in range(len(test_input)):
		
			fixed = test_input[i][1].squeeze()
			warped = test_output[i][0].squeeze()
			field = test_output[i][1]
			...
			fig, axes = ne.plot.slices(mid_slices_fixed + mid_slices_pred, cmaps=['gray'], do_colorbars=True, grid=[2,3],titles=titles)
	...
	def evaluate_loss_history(self, model_path=None, metric='eval'):
		...
		if model_path is None:
			...
			all_models = [Path(join(model_path, f)).name for f in listdir(model_path) if isdir(join(model_path, f)) and 'results' not in f]
		...
		models = [join(model_path, f)[:-6] for f in listdir(model_path) if isfile(join(model_path, f)) and "ckpt.index" in f]
