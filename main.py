...
try:
	...
	parser.add_argument('-c', '--case', required=True, default='testing', help="choose a case {intramodal T1w (=t1), intramodal T2 (=t2), intermodal T1w-T2w(t1t2), test mode (testing)}",type=str, choices=['t1', 't2', 't1t2', 'testing'])

	parser.add_argument('-pp', '--preprocessing', required=False, default=False, help="enable additional preprocessing",type=bool, choices=[True, False])

	parser.add_argument('-rp', '--retrain_path', required=False, default=None, help="path to previous trained model",type=str)
	
	parser.add_argument('-l', '--regularization_loss', required=False, default='l2', help="define regularization loss {'l2', 'l1'}", choices=['l2', 'l1']) 
